"""
Round 4: experiments3.py found 5 base features (IsSameBank, Hour, FromIsNew,
ToIsNew, IsCrossBorderBank) that each individually help when removed from
pair_only. This round tests whether removing all 5 AT ONCE ("lean") still
helps (or whether interaction effects cancel the individual gains), plus an
even more extreme "minimal_core" that also drops the 3 "mixed" features
(DayOfWeek, FromCountry, ToCountry).
...
VARIANTS = {
    "lean": ["LogAmountUSD", "PaymentFormat", "IsSelfLoop", "DayOfWeek",
             "FromCountry", "ToCountry", "pair_prior_count"],
    "minimal_core": ["LogAmountUSD", "PaymentFormat", "IsSelfLoop", "pair_prior_count"],
    "pair_only_recheck3": PAIR_ONLY,
}
...
with open(OUT_DIR / "results_experiments4.json", "w", encoding="utf-8") as f:
    json.dump(all_variant_results, f, indent=2, ensure_ascii=False)

log(f"Done. Results written to {OUT_DIR / 'results_experiments4.json'}")
