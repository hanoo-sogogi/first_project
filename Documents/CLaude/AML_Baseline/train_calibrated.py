"""
Calibration-split test: does re-thresholding on a held-out calibration slice
(instead of the default 0.5) fix the base-rate asymmetry found in 3.16
(HI<->LI cross-dataset transfer collapses precision when threshold stays 0.5)?

Reuses the 3.17 merged model (train on HI-Medium+LI-Medium) since that's the
current best production candidate. The only pipeline change: the 60-80% window
that was previously discarded as a train/test leakage buffer is now RETAINED
as a calibration split -- same time boundaries as before, nothing thrown away.

For each target dataset (HI-Medium, LI-Medium, HI-Small -- HI-Small is an
unseen world, never trained on):
  1. Score the target's own calibration split (60-80%) with the merged model.
  2. Pick the F1-maximizing threshold on that calibration split.
  3. Score the target's own test split (80-100%) at BOTH threshold=0.5 (baseline)
     and the calibrated threshold, side by side.

This tests the practical production question directly: if you have access to a
modest labeled slice of the deployment-target stream (even without retraining),
does re-thresholding alone fix the precision collapse?
"""
import gc
import json
import time
...
def preprocess(name):
    ...
    # 60/80 boundaries unchanged from prior scripts -- the 60-80% slice that was
    # previously discarded as a leakage buffer is now kept as the calibration split.
...
def evaluate(y_true, scores, threshold, name):
    ...
    pr_auc = average_precision_score(y_true, scores)  # threshold-independent, same either way
...
for seed in SEEDS:
    ...
    for target_name in DATASETS:
        ...
        for mtype, model in (("xgb", xgb_model), ("lgb", lgb_model)):
            ...
            r_default = evaluate(yte, test_scores, 0.5, f"{target_name}_{mtype}_s{seed}_thr0.5")
            r_calib = evaluate(yte, test_scores, calib_thr, f"{target_name}_{mtype}_s{seed}_thrCAL")
            ...
            results.append(row)
            with open(OUT_DIR / "results_calibrated.json", "w", encoding="utf-8") as f:
                json.dump({"merge_sources": MERGE_SOURCES, "features": FEATURES,
                           "results": results}, f, indent=2, ensure_ascii=False)

log(f"Done. Results written to {OUT_DIR / 'results_calibrated.json'}")
