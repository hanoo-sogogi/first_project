"""
Round 5: experiments4.py found that removing all 5 "individually helpful"
base features at once ("lean") introduces severe seed instability (both
models collapsing together on the same seed), worse than pair_only. This
round narrows down where that instability threshold is, by removing 2, 3,
then 4 of those 5 features incrementally (in order of individual impact
from experiments3.py: IsCrossBorderBank, IsSameBank, Hour, FromIsNew).
...
VARIANTS = {
    "remove2": drop("IsSameBank", "IsCrossBorderBank"),
    "remove3": drop("IsSameBank", "IsCrossBorderBank", "Hour"),
    "remove4": drop("IsSameBank", "IsCrossBorderBank", "Hour", "FromIsNew"),
}
...
with open(OUT_DIR / "results_experiments5.json", "w", encoding="utf-8") as f:
    json.dump(all_variant_results, f, indent=2, ensure_ascii=False)

log(f"Done. Results written to {OUT_DIR / 'results_experiments5.json'}")
