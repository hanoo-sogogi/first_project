"""
Round 2: starting from the pair_only winner, add back ONE of the other
7 graph aggregate features at a time. Tests whether any of them carries
marginal signal on top of pair_prior_count, or whether they are all noise.

Preprocessing is identical to train_baseline_experiments.py (copy-pasted,
not imported, to keep this a standalone reproducible script).
"""
import json
import time
import warnings
...
OTHER_AGG = ["out_degree", "out_unique_cp", "out_avg_amt", "in_degree",
             "in_unique_cp", "in_avg_amt", "in_unique_banks"]

# Round 2: pair_only + one extra graph feature at a time
VARIANTS = {f"pair_plus_{feat}": PAIR_ONLY + [feat] for feat in OTHER_AGG}
# also test: pair_only with ALL remaining features added back two at a time is out of scope;
# additionally test pair_only alone again here (seed-robustness re-check, same downsample idx)
VARIANTS["pair_only_recheck"] = PAIR_ONLY
...
with open(OUT_DIR / "results_experiments2.json", "w", encoding="utf-8") as f:
    json.dump(all_variant_results, f, indent=2, ensure_ascii=False)

log(f"Done. Results written to {OUT_DIR / 'results_experiments2.json'}")
