"""
Round 6: robustness check. experiments5.py found "remove2" (pair_only minus
IsSameBank, IsCrossBorderBank; 10 features) to be stable and the best result
across seeds 1-3. But experiments5.py also showed that neighboring 9/8-feature
configs can look fine on some seeds and collapse on others - so seeds 1-3
alone aren't strong enough evidence that remove2 is truly safe. This round
reruns remove2 (and pair_only as a control) on 3 NEW seeds (4, 5, 6) to check
whether the stability holds up out of sample.
...
def drop(*names):
    return [f for f in PAIR_ONLY if f not in names]


VARIANTS = {
    "pair_only_newseeds": PAIR_ONLY,
    "remove2_newseeds": drop("IsSameBank", "IsCrossBorderBank"),
}
...
with open(OUT_DIR / "results_experiments6.json", "w", encoding="utf-8") as f:
    json.dump(all_variant_results, f, indent=2, ensure_ascii=False)

log(f"Done. Results written to {OUT_DIR / 'results_experiments6.json'}")
