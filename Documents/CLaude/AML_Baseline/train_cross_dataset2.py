"""
Continuation of train_cross_dataset.py: covers the 5 combinations left
unfinished when that run was stopped (see AML_실험기록.md 3.16).
  - LI-Medium -> LI-Medium (within, redo all 6 -- only 1/6 had finished)
  - LI-Medium -> HI-Small  (cross)
  - HI-Small  -> HI-Medium (cross)
  - HI-Small  -> LI-Medium (cross)
  - HI-Small  -> HI-Small  (within)

Same preprocessing/evaluation protocol as train_cross_dataset.py (full test
sets, no sampling) so results are directly comparable to the 25 already
recorded. HI-Medium is preprocessed here too because it's needed as a
TARGET (for HI-Small->HI-Medium) even though we don't retrain on it as a
source (those 6 within/HI-Medium-source evals are already done).
"""
import gc
import json
import time
...
SOURCES = ["LI-Medium", "HI-Small"]  # only these need training this round
SEEDS = [1, 2, 3]
NEG_CAP = 400_000
...
for name in DATASETS:
    ...
    with open(OUT_DIR / "_cross_ds2_checkpoint.json", "w") as f:
...
for source, target in PAIRS:
    ...
    for seed in SEEDS:
        for mtype in ("xgb", "lgb"):
            ...
            r["kind"] = tag
            results.append(r)
            with open(OUT_DIR / "results_cross_dataset2.json", "w", encoding="utf-8") as f:
                json.dump({"pairs": PAIRS, "features": FEATURES, "results": results}, f, indent=2, ensure_ascii=False)

log(f"Done. Results written to {OUT_DIR / 'results_cross_dataset2.json'}")
