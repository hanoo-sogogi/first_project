"""
Round 3: leave-one-out ablation of the 11 "base" features on top of the
pair_only winner (BASE + pair_prior_count, 12 features total). Tests
whether the "less is more" pattern found among the 8 graph aggregates
(experiments.py / experiments2.py) also applies to the base feature set,
or whether all 11 base features are actually load-bearing.

Preprocessing is identical to train_baseline_experiments.py / 2.py
...
# Round 3: leave-one-out on the 11 base features (pair_prior_count always kept)
VARIANTS = {f"pair_only_minus_{feat}": [f for f in PAIR_ONLY if f != feat] for feat in BASE}
VARIANTS["pair_only_recheck2"] = PAIR_ONLY
...
with open(OUT_DIR / "results_experiments3.json", "w", encoding="utf-8") as f:
    json.dump(all_variant_results, f, indent=2, ensure_ascii=False)

log(f"Done. Results written to {OUT_DIR / 'results_experiments3.json'}")
